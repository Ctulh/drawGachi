  ...
  void draw (std::vector<std::string>& text) const override{
    for(int i=0; i<text.size(); i++){
      for(int j=0; j<text[i].size(); j++){
        ...
        {std::cout<< "\033[" << OutputManager::get_instance().getCurrentColor();
          OutputManager::get_instance().nextColor();
        }
        else
          std::cout << "\033[" << 37;
...
int main() {
  ...
  while(fgets(buffer, bufferLength, filePointer)) {
    OutputManager::get_instance().text.push_back(buffer);
    index++;
  }
  fclose(filePointer);
  ...
  main_loop.run(OutputManager::get_instance().text);
  t.join();
